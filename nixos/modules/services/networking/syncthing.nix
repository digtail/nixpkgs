...
let
  ...
  # function `curlAddressArgs` returns a given path the correct string to make
  # curl connect over either http(s) or unix. string which tells curl how to
  # connect to the address. this works for https and unix.
  curlAddressArgs = path: let
    matchResult = builtins.match "^unix://(.*)" cfg.guiAddress;
  in
    if matchResult == null
    # for http and https we do nothing
    then "${cfg.guiAddress}/${path}"
    ...
    else "--unix-socket ${builtins.head matchResult} .${path}"
    ;
